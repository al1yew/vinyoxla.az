...
export function LoginForm({ locale }: { locale: Locale }) {
  ...
  async function handleVerify(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await verifyOtp(toE164AzPhone(digits), parsedOtp.data, { locale });
  ...
  return (
    <Card className="mx-auto w-full max-w-xl p-5 sm:p-7">
      <div className="mb-6 text-center">
        ...
        <h1 className="text-2xl font-black text-slate-950 dark:text-white">{t("auth.title")}</h1>
        <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{t("auth.subtitle")}</p>
      </div>

      {step === "phone" ? (
        <form onSubmit={handleSendCode} className="grid gap-4">
          <label className="text-sm font-bold text-slate-700 dark:text-slate-200" htmlFor="phone">
            {t("auth.phoneLabel")}
          </label>
          <div className="flex rounded-lg border border-slate-200 bg-white focus-within:border-blue-500 dark:border-white/10 dark:bg-white/8">
          ...
          <Button type="submit" size="lg" disabled={loading} icon={loading ? <Spinner /> : <Phone className="h-5 w-5" aria-hidden="true" />}>
        ...
        <form onSubmit={handleVerify} className="grid gap-4">
          ...
          <Input
            id="otp"
            value={otp}
            onChange={(event) => setOtp(digitsOnly(event.target.value).slice(0, 4))}
          ...
          />
          <Button type="submit" size="lg" disabled={loading} icon={loading ? <Spinner /> : <KeyRound className="h-5 w-5" aria-hidden="true" />}>
